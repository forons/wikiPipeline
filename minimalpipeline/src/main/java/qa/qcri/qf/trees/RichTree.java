	...
	public static TokenTree getPosChunkTree(JCas cas) {
		...
		for (Sentence sentence : JCasUtil.select(cas, Sentence.class)) {
			...
			for (Chunk chunk : JCasUtil.selectCovered(cas, Chunk.class,
					sentence)) {
//				i++;
//				System.out.println("CHUNK" + i + ": " + chunk.getCoveredText());
...
//					System.out.println(token.getCoveredText());
					if(JCasUtil.contains(cas, token, WikipediaToken.class)) {
						for (WikipediaToken wikiToken : JCasUtil.selectCovered(
								cas, WikipediaToken.class, token.getBegin(),
								token.getEnd())) {
							...
							// wikiToken.getBegin()+ "\t" + wikiToken.getEnd());
							// Marker.addWikipediaTag(tokenNode, new MarkT());
						}
					}
						
					
//					System.out.println(tokenNode.getValue());

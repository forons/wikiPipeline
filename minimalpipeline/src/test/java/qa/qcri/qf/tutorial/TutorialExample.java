...
import qa.qcri.qf.treemarker.MarkTreesOnWikipedia;
...
public class TutorialExample {
	...
	public static void main(String[] args) throws UIMAException, IOException {
		...
		Analyzable passage = new SimpleContent(
				"tutorial-passage",
		...
		/**
		 * Analyze the question and the passage with different sets of
		 * annotators
		 */
		...
		/**
		 * Compute some features using the default ones instantiated by the
		 * factory
		 */
		PairFeatureFactory pf = new PairFeatureFactory(new Alphabet());
		FeatureVector fv = pf.getPairFeatures(questionCas, passageCas,
				parameterList);
		...
		/**
		 * Applying pruning with radius = 2 to the passage tree. Chunk nodes
		 * without RELATIONAL information distant two chunk nodes from the ones
		 * having RELATIONAL information, are pruned
		 */

		PosChunkPruner pruner = new PosChunkPruner(2);
		pruner.prune(passageTree, new PruneIfNodeIsWithoutMetadata(
				RichNode.REL_KEY));
	...
	public static Analyzer instantiateTrecAnalyzer(UIMAPersistence persistence)
		...
		AnalysisEngine wikipediaParser = AnalysisEngineFactory
				.createEngine(createEngineDescription(WikipediaAnnotator.class,
		 ...
		 */
		ae.addAE(stanfordSegmenter).addAE(stanfordPosTagger)
				.addAE(stanfordLemmatizer).addAE(illinoisChunker)
				.addAE(stanfordNamedEntityRecognizer).addAE(wikipediaParser);
		...
		ae.addAE(stanfordSegmenter, QUESTION_ANALYSIS)
				...
				.addAE(wikipediaParser, QUESTION_ANALYSIS);

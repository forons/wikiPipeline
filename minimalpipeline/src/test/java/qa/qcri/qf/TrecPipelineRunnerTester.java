...
import qa.qcri.qf.datagen.rr.RerankingTestOnlyWiki;
import qa.qcri.qf.datagen.rr.RerankingTestRelAndWiki;
import qa.qcri.qf.datagen.rr.RerankingTrainOnlyWiki;
import qa.qcri.qf.datagen.rr.RerankingTrainRelAndWiki;
...
import qa.qcri.qf.pipeline.trec.TrecCandidatesReader;
...
public class TrecPipelineRunnerTester {
...
//					new MarkTwoAncestors()).useStopwords(stoplist);
			...
			if (!new File(trainCandidatesPath + ".idf").exists()) {
				...
				CharacterNGramIdf.saveModel(ifdModel, trainCandidatesPath
						+ ".idf");
			} else {
			...
			pipeline.setupAnalysis(ae, new TrecQuestionsReader(
					trainQuestionsPath), new TrecCandidatesReader(
					trainCandidatesPath));

			if (!(trainCasesPath != null && cmd
					.hasOption(SKIP_SERIALIZATION_CHECK_OPT))) {
...
//					.setParameterList(parameterList);
			...
			pipeline.setupAnalysis(ae, new TrecQuestionsReader(
					testQuestionsPath), new TrecCandidatesReader(
					testCandidatesPath));

			if (!(testCasesPath != null && cmd
					.hasOption(SKIP_SERIALIZATION_CHECK_OPT))) {
				pipeline.performAnalysis();
			}

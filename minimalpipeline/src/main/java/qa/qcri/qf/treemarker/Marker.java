...
public class Marker {
	...
	/**
	 * Adds a wikipedia tag to the node selected by the given marking strategy
	 * 
	 * @param node
	 *            the starting node
	 * @param strategy
	 *            the marking strategy
	 */
	public static void addWikipediaTag(RichNode node, MarkingStrategy strategy) {
		for (RichNode nodeToMark : strategy.getNodesToMark(node)) {
			nodeToMark.getMetadata().put(RichNode.WIKI_KEY, RichNode.WIKI_KEY);
		}
	}
	
	/**
	 * Removes wikipedia information from a tree
	 * 
	 * @param tree the tree to clear
	 */
	public static void removeWikipediaTagFromTree(RichNode tree) {
		for (RichNode node : TreeUtil.getNodes(tree)) {
			node.getMetadata().remove(RichNode.WIKI_KEY);
		}
	}
